...

{
  ...
  if((p->pl->f) && !(p->pr->f)){
    ...
    //exit(10);
    return 1; // return as if an envelope violation
  }
...

{
  ...
  if(q->f){
    ...
    //exit(30);
    return 1; // "as if" an envelope violation
  }
...

{
  ...
  if(q->pl == NULL){
    ...
    //exit(1);
    a = 0.; // "as if" zero length interval
  } else if(q->pl->x == q->x){
