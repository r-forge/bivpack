...
typedef struct funbag { /* everything for evaluating log density          */
  ...
  double *mydata;   /* user-defined structure holding data for density */  
  double (*myfunc)(double x, double *mydata);
...
//void sample(ENVELOPE *env, POINT *p); // return value for error propagation
int sample(ENVELOPE *env, POINT *p);

//void invert(double prob, ENVELOPE *env, POINT *p); // ditto
int invert(double prob, ENVELOPE *env, POINT *p);
